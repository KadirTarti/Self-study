...
//! sadece listeleme istediği için forEach ideal. farklı dizeye aktarma isteseydi map metodu kullanılabilirdi

// userTodos.forEach((user) => {
//     console.log(user.title + " : " + user.completed);
//   });

//^*   forEach -> Array içinde işlem yapar. Her bir eleman içinde işlem gerçekleştirir
//^*   forEach -> Array içinde işlem yapar. Her bir eleman içinde işlem gerçekleştirir
//^*   forEach -> Array içinde işlem yapar. Her bir eleman içinde işlem gerçekleştirir
//^*   forEach -> Array içinde işlem yapar. Her bir eleman içinde işlem gerçekleştirir

//?       b- Birinci kullanıcının todo bilgisi ve completed durumunu listele

// userTodos
//     .filter((user1) => user1.userId === 1)
//     .forEach((user1) => console.log(`${user1.title} : ${user1.completed}`));

//?       c- Birinci kullanıcının completed: true durumunu listele

//   userTodos
//   .filter((user) => user.userId === 1)
//   .filter((user) => user.completed === true)
//   .forEach((user) => console.log(user.completed + "  ->  " + user.title));

//?       d- kullanıcnının girdiği id'ye sahip kişinin bitirdiği işlerin listesi

//   const id = +prompt("Enter an ID");

// const filteredTodos = userTodos.filter((x) => x.userId == id && x.completed == true);
// filteredTodos.forEach((filteredTodo) => {console.log(filteredTodo.title);});

//?       e - bu listeye yeni bir eleman ekleyelim
...
//?   products verisinden ürün ve fiyatlarını listele

products.forEach((productsParts) => {
  console.log(
    `product: ${productsParts.title} and it's ${productsParts.price} dolars.`
  );
});

//! js ten html e yazdırma... document.write
// products.forEach((productsParts) => document.write(`${productsParts.title} </br> <img src=${productsParts.thumbnail} </br>`))



//& destructure yöntemi ile :
products.forEach(({ title, price }) => {
  console.log(title, price);
});

//?   ilk ürünün tüm bilgilerini object destructure yöntemi ile göster

const firstProduct = products[0];
const { title, description, price, category } = firstProduct;
...
//?     laptopların adını ve fiyatlarını listele

products
.filter(({ category }) =>  category == "laptops")
.forEach(({ title, price}) => {
  console.log(`${title} cost just ${price} dollars.`);
});

//& or 
products
  .filter((filteredLaptops) => filteredLaptops.category === "laptops")
  .forEach((filteredLaptops) => console.log(`${filteredLaptops.title}: ${filteredLaptops.price}`));
...
//&kolay
// const newProducts = products.filter((prod) => prod.category !== "home-decoration")
...
//?     e- kullanıcının seçtiği ürünü (id'si girilmiş olan ürünü) silelim

// const deleteID = +prompt("Enter an ID (1-30");
...
//?    f- yeni ürün ekleyelim

const addProduct = {
        id: products.length+1,
        title: "iPhone 3",
        description: "An apple mobile which is nothing like watermelon",
        price: 50,
        discountPercentage: 70.22,
        rating: 3.78,
        stock: 5,
        brand: "Apple",
        category: "smartphones",
...
//?        g- rest operatörünü kullanarak fiyatlarda yazan oranlarda indirim yaptıralım
...
//?     h- her kategoride kaç ürün olduğunu gösterin

let kategoriUrun = products.reduce((toplam, urun) => {
    if (!toplam[urun.category]) {
        toplam[urun.category] = 0;
    }
    toplam[urun.category]++;
    return toplam;
}, {});

console.log(kategoriUrun);